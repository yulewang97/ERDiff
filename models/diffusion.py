...
def get_torch_trans(heads=8, layers=1, channels=64, ff_dim=256):
    encoder_layer = nn.TransformerEncoderLayer(
        d_model=channels, nhead=heads, dim_feedforward=ff_dim, activation="gelu"
    )
...
def sinusoidal_positional_encoding(seq_len, dim, device, dtype):
    ...
    half_dim = dim // 2
    if half_dim == 0:
        return torch.zeros(seq_len, dim, device=device, dtype=dtype)

    exponents = torch.arange(half_dim, device=device, dtype=dtype)
    div_term = torch.exp(-math.log(10000.0) * exponents / max(half_dim - 1, 1))
...
class STBlock(nn.Module):
    def __init__(self, channels, diffusion_embedding_dim, nheads, ff_dim):
        super().__init__()
        ...
        self.temporal_layer = get_torch_trans(heads=nheads, layers=1, channels=channels, ff_dim=ff_dim)
        self.spatio_layer = get_torch_trans(heads=nheads, layers=1, channels=channels, ff_dim=ff_dim)

    def forward_temporal(self, y, base_shape):
        B, channel, K, L = base_shape
        y = y.reshape(B, channel, K, L).permute(0, 2, 1, 3).reshape(B * K, channel, L)
        y = y.permute(2, 0, 1)
        temporal_pe = sinusoidal_positional_encoding(L, channel, y.device, y.dtype).unsqueeze(1)
        y = self.temporal_layer(y + temporal_pe).permute(1, 2, 0)
    ...
    def forward_spatio(self, y, base_shape):
        ...
        y = y.permute(2, 0, 1)
        spatio_pe = sinusoidal_positional_encoding(K, channel, y.device, y.dtype).unsqueeze(1)
        y = self.spatio_layer(y + spatio_pe).permute(1, 2, 0)
...
class Diff_STDiT(nn.Module):
    def __init__(
        ...
        self.diff_embedding_dim = diff_embedding_dim
        self.diff_layers = diff_layers
        self.diff_nheads = diff_nheads
        self.num_classes = num_classes
        self.ff_dim = ff_dim if ff_dim is not None else self.channels * 4

        self.diffusion_embedding = DiffusionEmbedding(
            num_steps=diff_timesteps,
            embedding_dim=self.diff_embedding_dim,
        )
        # For unconditional diffusion we do not use label embeddings.
        ...
        self.STBlock_layers = nn.ModuleList(
            [
                STBlock(
                    ...
                    diffusion_embedding_dim=self.diff_embedding_dim,
    ...
    def _process_labels(self, labels, batch_size, device):
        # Unconditional model: ignore any provided labels and return None.
        return None

    def forward(self, x, diffusion_step, labels=None):
        # External format: (B, C, length, feature). Internal blocks expect (B, C, K, L)
        # with temporal attention along L, so swap to (B, C, feature, length) here.
        x = x.permute(0, 1, 3, 2)
        ...
        labels = self._process_labels(labels, B, x.device)
        if labels is not None:
            diffusion_emb = diffusion_emb + self.label_embedding(labels)
        ...
        x = x.reshape(B, 1, K, L)
        # Convert back to external format: (B, C, length, feature)
...
def p_losses(denoise_model, x_start, t, noise=None, labels=None):
    if noise is None:
        noise = torch.randn_like(x_start)
    ...
    loss = 0.5 * F.l1_loss(predicted_noise, noise) + 0.5 * F.mse_loss(predicted_noise, noise)
...
@torch.no_grad()
def p_sample_loop(model, shape, labels=None, save_every=10):
    ...
    for i in tqdm(reversed(range(0, diff_timesteps)), desc='sampling loop time step', total=diff_timesteps):
        ...
        if i % save_every == 0 or i == 0:
            snapshots.append(img.detach().cpu().numpy())

    return img.cpu().numpy(), snapshots
